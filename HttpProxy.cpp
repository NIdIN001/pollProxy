            ...
            if (pollresult.fd == serverSocket.fd && pollresult.fd & (POLLIN | POLLPRI)) {
                ...
                for (int i = 0; i < proxyEntries.size(); i++) {
                    ...
                    if (proxyEnrty.clinetSocket->fd == pollresult.fd ||
                        ...
                        if (proxyEnrty.hostSocket != nullptr) {
        ...
        for (ProxyEntry &proxyEntry : proxyEntries) {
            if (proxyEntry.clientSocketHandler == clientSocketHandler) {
                ...
                if (proxyEntry.hostSocket == nullptr) {
                    ...
                    proxyEntry.hostSocket->_connect(hostName, DefaultDestPort);
                    ...
                    std::cout << "Adding host socket: fd = " << proxyEntry.hostSocket->fd << ", hostname = " << hostName
                              << ", pair client socket = " << proxyEntry.clinetSocket->fd << std::endl;


                    clientSocketHandler->setHostSocket(proxyEntry.hostSocket);

                    proxyEntry.hostSocketHandler = new HostSocketHandler(proxyEntry.clinetSocket, proxyEntry.hostSocket,
                                                                         cache, this);
